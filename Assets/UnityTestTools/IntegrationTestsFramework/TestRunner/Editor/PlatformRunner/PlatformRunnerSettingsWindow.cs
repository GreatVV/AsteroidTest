...
namespace UnityTest.IntegrationTests
{
    ...
    public class PlatformRunnerSettingsWindow : EditorWindow
    {
        ...
        private readonly GUIContent m_Label = new GUIContent(
            "Results target directory",
            "Directory where the results will be saved. If no value is specified, the results will be generated in project's data folder.");
        ...
        public PlatformRunnerSettingsWindow()
        {
            ...
            m_SceneList =
                Directory.GetFiles(Directory.GetCurrentDirectory(), "*.unity", SearchOption.AllDirectories).ToList();
            m_SceneList.Sort();
            string currentScene =
                (Directory.GetCurrentDirectory() + EditorApplication.currentScene).Replace("\\", "").Replace("/", "");
            IEnumerable<string> currentScenePath =
                m_SceneList.Where(s => s.Replace("\\", "").Replace("/", "") == currentScene);
        ...
        public void OnGUI()
        {
            ...
            foreach (string scenePath in m_SceneList)
            {
                string path = Path.GetFileNameWithoutExtension(scenePath);
                var guiContent = new GUIContent(path, scenePath);
                Rect rect = GUILayoutUtility.GetRect(guiContent, EditorStyles.label);
            ...
            DrawSetting();
            bool build = GUILayout.Button("Build and run tests");
        ...
        private void DrawSetting()
        {
            ...
            {
                Uri uri;
                if (!Uri.TryCreate(m_Settings.resultsPath, UriKind.Absolute, out uri) || !uri.IsFile ||
            ...
            m_Settings.sendResultsOverNetwork = EditorGUILayout.Toggle(
                                                                       "Send results to editor",
                                                                       m_Settings.sendResultsOverNetwork);
            EditorGUI.BeginDisabledGroup(!m_Settings.sendResultsOverNetwork);
            m_AdvancedNetworkingSettings = EditorGUILayout.Foldout(
                                                                   m_AdvancedNetworkingSettings,
                                                                   "Advanced network settings");
            if (m_AdvancedNetworkingSettings)
            {
                m_SelectedInterface = EditorGUILayout.Popup(
                                                            "Network interface",
                                                            m_SelectedInterface,
                                                            m_Interfaces.ToArray());
                EditorGUI.BeginChangeCheck();
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    {
                        if (m_Settings.port < IPEndPoint.MinPort)
                        {
