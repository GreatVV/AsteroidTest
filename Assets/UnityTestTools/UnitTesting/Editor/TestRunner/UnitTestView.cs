...
namespace UnityTest
{
    ...
    public partial class UnitTestView : EditorWindow
    {
        ...
        [SerializeField]
        private string[] m_AvailableCategories;

        [SerializeField]
        private List<string> m_FoldMarkers = new List<string>();

        [SerializeField]
        private List<UnitTestResult> m_ResultList = new List<UnitTestResult>();

        [SerializeField]
        private List<UnitTestRendererLine> m_SelectedLines = new List<UnitTestRendererLine>();

        private UnitTestsRunnerSettings m_Settings;
        private UnitTestRendererLine m_TestLines;

        #region GUI Contents

        private readonly GUIContent m_GUIAutoSaveSceneBeforeRun = new GUIContent(
            "Autosave scene",
            "The runner will automatically save the current scene changes before it starts");
        ...
        private readonly GUIContent m_GUIRunOnRecompile = new GUIContent(
            "Run on recompile",
            "Run all tests after recompilation");

        private readonly GUIContent m_GUIRunSelectedTestsIcon = new GUIContent(Icons.RunImg, "Run selected tests");

        private readonly GUIContent m_GUIRunTestsOnNewScene = new GUIContent(
            "Run tests on a new scene",
            "Run tests on a new scene");

        private readonly GUIContent m_GUIShowDetailsBelowTests = new GUIContent(
            "Show details below tests",
            "Show run details below test list");
        ...
        public void OnGUI()
        {
            ...
            {
                m_TestLines.RunTests(
                                     m_ResultList.Where(result => result.IsFailure || result.IsError)
                                                 .Select(l => l.FullName)
                                                 .ToArray());
            }
            ...
            if (GUILayout.Button(
                                 m_Settings.optionsFoldout ? m_GUIHideButton : m_GUIOptionButton,
                                 GUILayout.Height(24),
                                 GUILayout.Width(80)))
            {
            ...
            {
                m_Settings.categoriesMask = EditorGUILayout.MaskField(
                                                                      m_Settings.categoriesMask,
                                                                      m_AvailableCategories,
                                                                      GUILayout.MaxWidth(90));
            }

            if (GUILayout.Button(
                                 m_Settings.filtersFoldout ? "Hide" : "Advanced",
                                 GUILayout.Width(80),
                                 GUILayout.Height(15)))
            {
        ...
        private string[] GetSelectedCategories()
        {
            ...
            foreach (string availableCategory in m_AvailableCategories)
            {
                int idx = Array.FindIndex(m_AvailableCategories, a => a == availableCategory);
                int mask = 1 << idx;
                if ((m_Settings.categoriesMask & mask) != 0)
                {
        ...
        private void DrawFilters()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            m_Settings.showSucceeded = GUILayout.Toggle(
                                                        m_Settings.showSucceeded,
                                                        m_GUIShowSucceededTests,
                                                        GUI.skin.FindStyle(GUI.skin.button.name + "left"),
                                                        GUILayout.ExpandWidth(true));
            m_Settings.showFailed = GUILayout.Toggle(
                                                     m_Settings.showFailed,
                                                     m_GUIShowFailedTests,
                                                     GUI.skin.FindStyle(GUI.skin.button.name + "mid"));
            m_Settings.showIgnored = GUILayout.Toggle(
                                                      m_Settings.showIgnored,
                                                      m_GUIShowIgnoredTests,
                                                      GUI.skin.FindStyle(GUI.skin.button.name + "mid"));
            m_Settings.showNotRun = GUILayout.Toggle(
                                                     m_Settings.showNotRun,
        ...
        private void DrawOptions()
        {
            ...
            m_Settings.runTestOnANewScene = EditorGUILayout.Toggle(
                                                                   m_GUIRunTestsOnNewScene,
                                                                   m_Settings.runTestOnANewScene);
            EditorGUI.BeginDisabledGroup(!m_Settings.runTestOnANewScene);
            m_Settings.autoSaveSceneBeforeRun = EditorGUILayout.Toggle(
                                                                       m_GUIAutoSaveSceneBeforeRun,
                                                                       m_Settings.autoSaveSceneBeforeRun);
        ...
        private void RefreshTests()
        {
            ...
            foreach (UnitTestResult newResult in newResults)
            {
                UnitTestResult[] result =
                    m_ResultList.Where(t => t.Test == newResult.Test && t.FullName == newResult.FullName).ToArray();
                if (result.Count() != 1)
                {
                    continue;
                }
