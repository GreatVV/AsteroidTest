...
namespace UnityTest
{
    ...
    {
        private readonly GUILayoutOption[] m_ButtonLayoutOptions =
        {
        ...
        {
            Draw(null, selected, options, onValueSelected);
        }
        ...
        public void Draw(string label, T selected, Func<T[]> loadOptions, Action<T> onValueSelected)
        {
            ...
            if (GUILayout.Button(
                                 new GUIContent(convertForButtonLabel(selected), tooltip),
                                 EditorStyles.popup,
                                 m_ButtonLayoutOptions))
            {
                ...
                {
                    if (printContextMenu != null && Event.current.button == 1)
                    {
        ...
        public void PrintMenu(T[] options)
        {
            ...
            foreach (T s in options)
            {
                ...
                menu.AddItem(
                             new GUIContent(
                                 (ignoreConvertForGUIContent(options) ? localS.ToString() : convertForGUIContent(localS))),
                             false,
                             () =>
                             {
