...
namespace UnityTest
{
    ...
    public abstract class AssertionListRenderer<T> : IListRenderer
    {
        ...
        public void Render(IEnumerable<AssertionComponent> allAssertions, List<string> foldMarkers)
        {
            foreach (var grouping in GroupResult(allAssertions))
            {
                ...
                if (key != "")
                {
                    ...
                    isFolded = PrintFoldout(isFolded, grouping.Key);
                ...
                foreach (AssertionComponent assertionComponent in grouping)
                {
                    ...
                    EditorGUI.BeginChangeCheck();
                    if (GUILayout.Button("", EditorStyles.foldout, GUILayout.Width(15)))
                    {
        ...
        protected virtual bool PrintFoldout(bool isFolded, T key)
        {
            ...
            Rect rect = GUILayoutUtility.GetRect(content, EditorStyles.foldout, GUILayout.MaxWidth(size.x));
            bool res = EditorGUI.Foldout(rect, !isFolded, content, true);
        ...
        protected virtual void PrintFoldedAssertionLine(AssertionComponent assertionComponent)
        {
            ...
            PrintPath(assertionComponent.Action.go, assertionComponent.Action.thisPropertyPath);
            EditorGUILayout.EndHorizontal();
            EditorGUILayout.EndVertical();
            ...
            if (assertionComponent.Action is ComparerBase)
            {
                ...
                string otherStrVal = "(no value selected)";
                EditorGUILayout.BeginVertical();
                EditorGUILayout.BeginHorizontal(GUILayout.MaxWidth(300));
                switch (comparer.compareToType)
                {
                    case ComparerBase.CompareToType.CompareToObject:
                        ...
                        {
                            PrintPath(comparer.other, comparer.otherPropertyPath);
                        }
                        else
                        {
                            EditorGUILayout.LabelField(otherStrVal, Styles.redLabel);
                        }
                        break;
        ...
        protected virtual void PrintAssertionLineDetails(AssertionComponent assertionComponent)
        {
            ...
            EditorGUILayout.BeginVertical(GUILayout.MaxWidth(250));
            EditorGUILayout.EnumMaskField(assertionComponent.checkMethods, EditorStyles.popup, GUILayout.MaxWidth(150));
            EditorGUILayout.EndVertical();

            EditorGUILayout.BeginVertical();
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField("Disabled", GUILayout.Width(55));
            assertionComponent.enabled = !EditorGUILayout.Toggle(!assertionComponent.enabled, GUILayout.Width(15));
            EditorGUILayout.EndHorizontal();
            EditorGUILayout.EndVertical();
        ...
        private void PrintPath(GameObject go, string propertyPath)
        {
            ...
            if (go != null)
            {
                ...
                EditorGUILayout.ObjectField(go, typeof (GameObject), true, GUILayout.Width(sss.x));
            ...
            var content = new GUIContent(contentString, contentString);
            Rect rect = GUILayoutUtility.GetRect(content, EditorStyles.label, GUILayout.MaxWidth(200));

            EditorGUI.LabelField(rect, content, styleThisPath);
        }
