...
namespace UnityTest
{
    public class PropertyPathSelector
    {
        ...
        public void Draw
            ...
            ActionBase comparer,
            string propertPath,
            Type[] accepatbleTypes,
            Action<GameObject> onSelectedGo,
            Action<string> onSelectedPath)
        {
            ...
            if (go != null)
            {
                string newPath = DrawListOfMethods(go, comparer, propertPath, accepatbleTypes, m_ThisDropDown);
        ...
        private string DrawListOfMethods
            (
            GameObject go,
            ActionBase comparer,
            string propertPath,
            Type[] accepatbleTypes,
            DropDownControl<string> dropDown)
        {
            ...
            else
            {
                ...
                {
                    List<string> options = GetFieldsAndProperties(go, comparer, result, accepatbleTypes);
                ...
                if (isPropertyOrFieldFound)
                {
                    dropDown.Draw(
                                  ...
                                  () =>
                                  {
                                      try
                                      {
                                          List<string> options = GetFieldsAndProperties(
                                                                                        go,
                                                                                        comparer,
                                                                                        result,
                                                                                        accepatbleTypes);
                                      ...
                                      {
                                          Debug.LogWarning(
                                                           "An exception was thrown while resolving a property list. Resetting property path.");
                                          result = "";
                                          return new string[0];
                                      }
                                  },
        ...
        private static List<string> GetFieldsAndProperties
            (GameObject go, ActionBase comparer, string extendPath, Type[] accepatbleTypes)
        {
            var propertyResolver = new PropertyResolver
                                   {
                                       AllowedTypes = accepatbleTypes,
                                       ExcludedFieldNames = comparer.GetExcludedFieldNames()
                                   };
            List<string> options =
                propertyResolver.GetFieldsAndPropertiesFromGameObject(go, comparer.GetDepthOfSearch(), extendPath)
                                .ToList();
            options.Sort(
                         (x, y) =>
                         {
        ...
        private string DrawManualPropertyEditField
            (GameObject go, string propertPath, Type[] acceptableTypes, DropDownControl<string> dropDown)
        {
            var propertyResolver = new PropertyResolver
                                   {
            ...
            Vector2 labelSize = EditorStyles.label.CalcSize(new GUIContent(go.name + '.'));
            GUILayout.Label(
                            go.name + (propertPath.Length > 0 ? "." : ""),
                            EditorStyles.label,
                            GUILayout.Width(labelSize.x));

            string btnName = "hintBtn";
            if (GUI.GetNameOfFocusedControl() == btnName && Event.current.type == EventType.KeyDown &&
            ...
            string result = GUILayout.TextField(propertPath, EditorStyles.textField);
            ...
            switch (m_Error)
            {
                ...
                case SelectedPathError.MissingComponent:
                    EditorGUILayout.HelpBox(
                                            "This property or field is not attached or set. It will fail unless it will be attached before the check is perfomed.",
                                            MessageType.Warning);
                    break;
            }
        ...
        #region Nested type: SelectedPathError
