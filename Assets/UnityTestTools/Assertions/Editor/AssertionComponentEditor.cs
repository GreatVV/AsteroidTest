...
namespace UnityTest
{
    ...
    public class AssertionComponentEditor : Editor
    {
        ...
        private readonly GUIContent m_GUICheckAfterFramesGuiContent = new GUIContent(
            "Check after (frames)",
            "After how many frames the assertion should be checked");

        private readonly GUIContent m_GUICheckAfterTimeGuiContent = new GUIContent(
            "Check after (seconds)",
            "After how many seconds the assertion should be checked");

        private readonly GUIContent m_GUIRepeatCheckFrameGuiContent = new GUIContent(
            "Repeat check",
            "Should the check be repeated.");

        private readonly GUIContent m_GUIRepeatCheckTimeGuiContent = new GUIContent(
            "Repeat check",
            "Should the check be repeated.");

        private readonly GUIContent m_GUIRepeatEveryTimeGuiContent = new GUIContent(
        ...
        public AssertionComponentEditor()
        {
            ...
            m_ComparerDropDown.tooltip =
                "Comparer that will be used to compare values and determine the result of assertion.";
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            bool obj = DrawComparerSelection(script);
            script.checkMethods =
                (CheckMethod)
                EditorGUILayout.EnumMaskField(script.checkMethods, EditorStyles.popup, GUILayout.ExpandWidth(false));
            ...
            if (obj)
            {
                ...
                m_ThisPathSelector.Draw(
                                        script.Action.go,
                                        script.Action,
                                        script.Action.thisPropertyPath,
                                        script.Action.GetAccepatbleTypesForA(),
                                        go =>
                                        {
                                            script.Action.go = go;
        ...
        {
            EditorGUILayout.Space();
            script.checkAfterTime = EditorGUILayout.FloatField(m_GUICheckAfterTimeGuiContent, script.checkAfterTime);
            if (script.checkAfterTime < 0)
            {
                script.checkAfterTime = 0;
            }
            script.repeatCheckTime = EditorGUILayout.Toggle(m_GUIRepeatCheckTimeGuiContent, script.repeatCheckTime);
            if (script.repeatCheckTime)
            {
                script.repeatEveryTime = EditorGUILayout.FloatField(
        ...
        {
            EditorGUILayout.Space();
            script.checkAfterFrames = EditorGUILayout.IntField(m_GUICheckAfterFramesGuiContent, script.checkAfterFrames);
            if (script.checkAfterFrames < 1)
            {
                script.checkAfterFrames = 1;
            }
            script.repeatCheckFrame = EditorGUILayout.Toggle(m_GUIRepeatCheckFrameGuiContent, script.repeatCheckFrame);
            if (script.repeatCheckFrame)
            {
                script.repeatEveryFrame = EditorGUILayout.IntField(
                                                                   m_GUIRepeatEveryTimeGuiContent,
        ...
        private void DrawCompareToType(ComparerBase comparer)
        {
            comparer.compareToType =
                (ComparerBase.CompareToType)
                EditorGUILayout.EnumPopup("Compare to type", comparer.compareToType, EditorStyles.popup);
            ...
            {
                if (comparer.compareToType == ComparerBase.CompareToType.CompareToObject)
                {
        ...
        {
            m_OtherPathSelector.Draw(
                                     comparer.other,
                                     comparer,
                                     comparer.otherPropertyPath,
                                     comparer.GetAccepatbleTypesForB(),
                                     go =>
                                     {
                                         comparer.other = go;
                                         AssertionExplorerWindow.Reload();
        ...
        private void DrawConstCompareField(ComparerBase comparer)
        {
            ...
            SerializedProperty sp = so.FindProperty("constantValueGeneric");
            if (sp != null)
            {
        ...
        {
            Type[] types = typeof (ActionBase).Assembly.GetTypes();
            Type[] allComparers =
                types.Where(type => type.IsSubclassOf(typeof (ActionBase)) && !type.IsAbstract).ToArray();
            ...
            m_ComparerDropDown.Draw(
                                    script.Action.GetType(),
                                    allComparers,
                                    type =>
                                    {
        ...
        private void DrawCustomFields(AssertionComponent script)
        {
                          ...
                          .GetFields(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly))
            {
                ...
                SerializedProperty sp = so.FindProperty(prop.Name);
                if (sp != null)
                {
